...
int init_infile(lame_global_flags * gfp, char *inPath, int *enc_delay, int *enc_padding)
{
    /* open the input file */
	...
	return (global.musicin != NULL);
}

int init_infile_for_buffer(lame_global_flags * gfp, BYTE *pMp3Data, int lenMp3Data, int *enc_delay, int *enc_padding)	// choi090326 add
{
	int readbytes = 0;
	global. count_samples_carefully = 0;
	global. num_samples_read = 0;
	...
	global. pcm_is_unsigned_8bit = in_signed == 1 ? 0 : 1;
	global. musicin = NULL;
	readbytes = OpenSndFileForBufferDecode(gfp, pMp3Data, lenMp3Data, &enc_delay, &enc_padding);
	return readbytes;
}
...
int get_audio16_FromBuffer(lame_global_flags * const gfp, short buffer[2][1152], BYTE *pMp3Data, int *pIdxMp3, int lenMp3)		// choi090326 add
{
    return (get_audio_common_FromBuffer(gfp, NULL, buffer, pMp3Data, pIdxMp3, lenMp3));
}
...
int get_audio_common_FromBuffer(lame_global_flags * const gfp, int buffer[2][1152], short buffer16[2][1152], BYTE *pMp3Data, int *pIdxMp3, int lenMp3)	// choi090326 add
{
    int     num_channels = lame_get_num_channels(gfp);
    int     insamp[2 * 1152];
    short   buf_tmp16[2][1152];
    ...
    int     i;
    int    *p;
	extern int read_samples_mp3_FromBuffer(lame_global_flags * const gfp, BYTE *pMp3Data, int *pIdxMp3, int nMp3Data, short int mpg123pcm[2][1152]);	// choi090326 add
    ...
    if (is_mpeg_file_format(input_format)) {
        if (buffer != NULL)
            ...
            samples_read = read_samples_mp3_FromBuffer(gfp, pMp3Data, pIdxMp3, lenMp3, buffer16);
        if (samples_read < 0) {
            return samples_read;
        }
    }
...
int get_audio_common_2(lame_global_flags * const gfp, const unsigned char *pcmdata, int len, int buffer[2][1152])	// choi081226 add
{
    int     num_channels = lame_get_num_channels(gfp);
    int     insamp[2 * 1152];
    short   buf_tmp16[2][1152];
    ...
    samples_read = read_samples_pcm_2(insamp, num_channels * samples_to_read, pcmdata, len);
    if (samples_read < 0) {
        return samples_read;
    }
    p = insamp + samples_read;
...
int read_samples_mp3_FromBuffer(lame_global_flags * const gfp, BYTE *pMp3Data, int *pIdxMp3, int nMp3Data, short int mpg123pcm[2][1152])	// choi090326 add
{
    int     out;
#if defined(AMIGA_MPEGA)  ||  defined(HAVE_MPGLIB)
    static const char type_name[] = "MP3 file";

	extern int lame_decode_FromBuffer(BYTE *pMp3Data, int *pIdxMp3, int nMp3Data, short pcm_l[], short pcm_r[], mp3data_struct * mp3data);
    out = lame_decode_FromBuffer(pMp3Data, pIdxMp3, nMp3Data, mpg123pcm[0], mpg123pcm[1], &mp3input_data);	// choi090326 modi
    /*
     * out < 0:  error, probably EOF
     * out = 0:  not possible with lame_decode_fromfile() ???
     * out > 0:  number of output samples
...
static int
unpack_read_samples_2(const int samples_to_read, const int bytes_per_sample,
                    const int swap_order, int *sample_buffer, const unsigned char *pcmdata, int pcmlen)	// choi081226 add
{
    size_t  samples_read;
    int     i;
    int    *op;              /* output pointer */
...
#define GA_URS_IFLOOP( ga_urs_bps ) \
    ...
    //samples_read = fread(sample_buffer, bytes_per_sample, samples_to_read, pcm_in);
	memcpy( sample_buffer, pcmdata, bytes_per_sample*samples_to_read );
...
static int
read_samples_pcm_2(int sample_buffer[2304], int samples_to_read, const unsigned char *pcmdata, int pcmlen )		// choi081226 add
{
    int     samples_read = -1;
    int     swap_byte_order;     /* byte order of input stream */
    ...
    case 16:
        ...
        }
        {
            ...
            samples_read = unpack_read_samples_2(samples_to_read, global.pcmbitwidth / 8,
                                               swap_byte_order, sample_buffer, pcmdata, pcmlen);
    ...
    case 8:
        {
            samples_read = unpack_read_samples_2(samples_to_read, 1, global.pcm_is_unsigned_8bit,
                                               sample_buffer, pcmdata, pcmlen);
        }
        break;
...
OpenSndFile(lame_global_flags * gfp, char *inPath, int *enc_delay, int *enc_padding)
{
	FILE   *musicin = NULL;		// choi090401 modi(=NULL 추가)
...
// pMp3Data에서 처리한 byte 수를 return.
int OpenSndFileForBufferDecode(lame_global_flags * gfp, BYTE *pMp3Data, int nMp3Data, int *enc_delay, int *enc_padding) // choi090326 add
{
    FILE   *musicin;
	int bytes = 0;
...
#ifdef HAVE_MPGLIB
    bytes = lame_decode_initfile_for_buffer(&mp3input_data, pMp3Data, nMp3Data, enc_delay, enc_padding);
#endif
    if (-1 == lame_set_num_channels(gfp, mp3input_data.stereo)) {
        if (silent < 10) {
            error_printf("Unsupported number of channels: %ud\n", mp3input_data.stereo);
...
// return : 읽은 byte 수.
int lame_decode_initfile_for_buffer(mp3data_struct * mp3data, BYTE *pMp3Data, int nMp3Data, int *enc_delay, int *enc_padding) // choi090326 add
{
    /*  VBRTAGDATA pTagData; */
    /* int xing_header,len2,num_frames; */
    unsigned char buf[100];
    ...
    len = 4;
	memcpy( buf, &pMp3Data[readbytes], len ); readbytes += len;
    while (!is_syncword_mp123(buf)) {
        unsigned int i;
        for (i = 0; i < len - 1; i++)
            buf[i] = buf[i + 1];
		memcpy( buf + len - 1, &pMp3Data[readbytes], 1 ); readbytes += 1;
    }
    ...
    /* repeat until we decode a valid mp3 header.  */
    while (!mp3data->header_parsed) {
		if ( (nMp3Data-readbytes) < bufsize )
			return 0;
		memcpy( buf, &pMp3Data[readbytes], bufsize ); readbytes += bufsize;
		len = bufsize;
        ret = lame_decode1_headersB(buf, len, pcm_l, pcm_r, mp3data, enc_delay, enc_padding);
        if (-1 == ret)
            return -1;
...
// return : read bytes.
int lame_decode_FromBuffer(BYTE *pMp3Data, int *pIdxMp3, int nMp3Data, short pcm_l[], short pcm_r[], mp3data_struct * mp3data)	// choi090326 add
{
    int     ret = 0;
    size_t  len = 0;
    unsigned char buf[1024];
    ...
    while (1) {
        ...
        //len = fread(buf, 1, 1024, fd);
		len = min( nMp3Data-(*pIdxMp3), 1024 );
		memcpy( buf, &pMp3Data[*pIdxMp3], len );
		*pIdxMp3 += len;
