...
init_files(lame_global_flags * gf, char *inPath, char *outPath, int *enc_delay, int *enc_padding)
{
     ...
     */
    if ( !init_infile(gf, inPath, enc_delay, enc_padding) )
	{
...
static lame_global_flags *s_gf = NULL;

int lame_PcmInit( int argc, char **argv )
{
    int     ret;
    char    outPath[PATH_MAX + 1];
    char    nogapdir[PATH_MAX + 1];
...
#define MAX_NOGAP 200
	...
	unsigned char mp3buffer[LAME_MAXMP3BUFFER];	// lame_encoder()에서 옮겨온 것. choi090101 add block
	int     Buffer[2][1152];
	int     iread, imp3, owrite, id3v2_size;
    ...
    /* initialize libmp3lame */
    input_format = sf_unknown;
    if (NULL == (s_gf = lame_init())) {
        error_printf("fatal error during initialization\n");
        frontend_close_console();
        return 1;
    }
    lame_set_errorf(s_gf, &frontend_errorf);
    lame_set_debugf(s_gf, &frontend_debugf);
    lame_set_msgf(s_gf, &frontend_msgf);
    if (argc <= 1) {
        usage(stderr, argv[0]); /* no command-line args, print usage, exit  */
        lame_close(s_gf);
        frontend_close_console();
    ...
    /* parse the command line arguments, setting various flags in the
     * struct 's_gf'.  If you want to parse your own arguments,
     * or call libmp3lame from a program which uses a GUI to set arguments,
     * skip this call and set the values of interest in the s_gf struct.
     * (see the file API and lame.h for documentation about these parameters)
     */
    parse_args_from_string(s_gf, getenv("LAMEOPT"), inPath, outPath);
    ret = parse_args(s_gf, argc, argv, inPath, outPath, nogap_inPath, &max_nogap);
    if (ret < 0) {
        lame_close(s_gf);
        frontend_close_console();
        return ret == -2 ? 0 : 1;
    ...
    if (max_nogap > 0) {
         ...
         * specify the output file name, only an optional output directory. */
        parse_nogap_filenames(nogapout, nogap_inPath[0], outPath, nogapdir);
        outf = init_files(s_gf, nogap_inPath[0], outPath, &enc_delay, &enc_padding);
    }
    else {
        outf = init_files(s_gf, inPath, outPath, &enc_delay, &enc_padding);
    }
    if (outf == NULL) {
        lame_close(s_gf);
        frontend_close_console();
     ...
     * function would spit out ID3v2 tag data.
     */
    lame_set_write_id3tag_automatic(s_gf, 0);

    /* Now that all the options are set, lame needs to analyze them and
     * set some more internal options and check for problems
     */
    i = lame_init_params(s_gf);
    if (i < 0) {
        if (i == -1) {
            display_bitrates(stderr);
        }
     ...
     * encode a single input file
     */
    brhist_init_package(s_gf);
    
	//////////////////////////////////////////////////////////
    //ret = lame_encoder(s_gf, outf, 0, inPath, outPath);
	//////////////////////////////////////////////////////////

	encoder_progress_begin(s_gf, inPath, outPath);

	imp3 = lame_get_id3v2_tag(s_gf, mp3buffer, sizeof(mp3buffer));
	if ((size_t)imp3 > sizeof(mp3buffer)) {
		encoder_progress_end(s_gf);
		error_printf("Error writing ID3v2 tag: buffer too small: buffer size=%d  ID3v2 size=%d\n"
				, sizeof(mp3buffer)
				, imp3
					);
...
int lame_PcmEncode( unsigned char *mp3data, const unsigned char *pcmdata, int len )
{
	int mp3bytes = 0;
	unsigned char mp3buffer[LAME_MAXMP3BUFFER];
	int     Buffer[2][1152];
	...
	/* read in 'iread' samples */
	//iread = get_audio(s_gf, Buffer);
	extern int get_audio_common_2(lame_global_flags * const gfp, const unsigned char *pcmdata, int len, int buffer[2][1152]);
	iread = get_audio_common_2(s_gf, pcmdata, len, Buffer);

	if (iread >= 0) {
		encoder_progress(s_gf);

		/* encode */
		imp3 = lame_encode_buffer_int(s_gf, Buffer[0], Buffer[1], iread,
									  mp3buffer, sizeof(mp3buffer));
		...
		if (imp3 < 0) {
			if (imp3 == -1)
				...
				error_printf("mp3 internal error:  error code=%i\n", imp3);
			mp3bytes = 0; goto _ret;
		}
		// choi081226 modi start
		/*owrite = (int) fwrite(mp3buffer, 1, imp3, outf);
		if (owrite != imp3) {
			error_printf("Error writing mp3 output \n");
			mp3bytes = 0; goto _ret;
		}*/
		memcpy( mp3data, mp3buffer, imp3 );
		mp3bytes = imp3;
		// choi081226 modi end
	}
	/*if (flush_write == 1) {		choi081226 del
		fflush(outf);
	}*/
_ret:;
	return mp3bytes;
...
int lame_PcmExit()
{
	...
	imp3 = lame_encode_flush(s_gf, mp3buffer, sizeof(mp3buffer)); /* may return one more mp3 frame */
	...
	if (imp3 < 0) {
		if (imp3 == -1)
			...
			error_printf("mp3 internal error:  error code=%i\n", imp3);
		ret = 1; goto _inner_ret;

	}

	encoder_progress_end(s_gf);
    
	/*owrite = (int) fwrite(mp3buffer, 1, imp3, outf);	choi081226 del
	if (owrite != imp3) {
		error_printf("Error writing mp3 output \n");
		ret = 1; goto _inner_ret;
	}
	...
	imp3 = lame_get_id3v1_tag(s_gf, mp3buffer, sizeof(mp3buffer));
	if ((size_t)imp3 > sizeof(mp3buffer)) {
		error_printf("Error writing ID3v1 tag: buffer too small: buffer size=%d  ID3v1 size=%d\n"
				, sizeof(mp3buffer)
				, imp3
	...
	if (silent <= 0) {
		print_lame_tag_leading_info(s_gf);
	}
	if (fseek(outf, id3v2_size, SEEK_SET) != 0) {
		error_printf("fatal error: can't update LAME-tag frame!\n");                    
	}
	else {
		write_xing_frame(s_gf, outf);
	}*/

	if (silent <= 0) {
		print_trailing_info(s_gf);
	}    
	ret = 0;
_inner_ret:;

	//fclose(outf); /* close the output file */		choi081226 del
    close_infile(); /* close the input file */
...
///////////////////////////////////////////////////////////////////////
// LAME customized decode pard. choi090325 add block

static lame_global_flags *g_gf;
static int g_skip_start = 0;
static int g_skip_end = 0;
static int (*g_WriteFunction)(BYTE *buffer, int index, char *p, int n);
static int g_tmp_num_channels = 0;

int WriteBytesSwappedToBuffer(BYTE *buffer, int index, char *p, int n)
{
	int len = n;
    p += n - 1;
    while (n-- > 0)
...
int WriteBytesToBuffer(BYTE *buffer, int index, char *p, int n)
{
	int len = n;
...
// return : -1 for error, else read bytes.
int lame_DecodeInit(int argc, char **argv, BYTE *pMp3Data, int lenMp3Data)
{
    int     ret;
    char    outPath[PATH_MAX + 1];
    char    nogapdir[PATH_MAX + 1];
...
#define MAX_NOGAP 200
    ...
    /* initialize libmp3lame */
    input_format = sf_unknown;
    if (NULL == (g_gf = lame_init())) {
        error_printf("fatal error during initialization\n");
        frontend_close_console();
        return -1;
    }
    lame_set_errorf(g_gf, &frontend_errorf);
    lame_set_debugf(g_gf, &frontend_debugf);
    lame_set_msgf(g_gf, &frontend_msgf);
    if (argc <= 1) {
        usage(stderr, argv[0]); /* no command-line args, print usage, exit  */
        lame_close(g_gf);
        frontend_close_console();
     ...
     * (see the file API and lame.h for documentation about these parameters)
     */
    parse_args_from_string(g_gf, getenv("LAMEOPT"), inPath, outPath);
    ret = parse_args(g_gf, argc, argv, inPath, outPath, nogap_inPath, &max_nogap);
    if (ret < 0) {
        lame_close(g_gf);
        frontend_close_console();
        return -1;
	...
	//
	{
		 ...
		 */
		/* open the input file */
		extern int init_infile_for_buffer(lame_global_flags * gfp, BYTE *pMp3Data, int lenMp3Data, int *enc_delay, int *enc_padding);
		readbytes = init_infile_for_buffer(g_gf, pMp3Data, lenMp3Data, &enc_delay, &enc_padding);
		//if ( !init_infile(gf, inPath, &enc_delay, &enc_padding) )
		//{
		//	error_printf("Can't init infile '%s'\n", inPath);
		//	return NULL;
		...
		if ((outf = init_outfile(outPath, lame_get_decode_only(g_gf))) == NULL) {
			error_printf("Can't init outfile '%s'\n", outPath);
			return -1;
		}
	}
     ...
     * function would spit out ID3v2 tag data.
     */
    lame_set_write_id3tag_automatic(g_gf, 0);

    /* Now that all the options are set, lame needs to analyze them and
     * set some more internal options and check for problems
     */
    i = lame_init_params(g_gf);
    if (i < 0) {
        if (i == -1) {
            display_bitrates(stderr);
        }
    ...
    //lame_decoder(g_gf, outf, 0, inPath, outPath, &enc_delay, &enc_padding);

	//
	// lame_decode() 중 loop 전 부분.
	//

    g_tmp_num_channels = lame_get_num_channels(g_gf);
    ...
    case sf_mp3:
        if (g_skip_start == 0) {
            if (enc_delay > -1 || enc_padding > -1) {
                if (enc_delay > -1)
                    g_skip_start = enc_delay + 528 + 1;
                if (enc_padding > -1)
                    g_skip_end = enc_padding - (528 + 1);
            }
            else
                g_skip_start = lame_get_encoder_delay(g_gf) + 528 + 1;
        }
        else {
            /* user specified a value of skip. just add for decoder */
            g_skip_start += 528 + 1; /* mp3 decoder has a 528 sample delay, plus user supplied "skip" */
        }

        if (silent < 10)
            console_printf("MPEG-%u%s Layer %s", 2 - lame_get_version(g_gf),
                           lame_get_out_samplerate(g_gf) < 16000 ? ".5" : "", "III");
        break;
    case sf_mp2:
        g_skip_start += 240 + 1;
        if (silent < 10)
            console_printf("MPEG-%u%s Layer %s", 2 - lame_get_version(g_gf),
                           lame_get_out_samplerate(g_gf) < 16000 ? ".5" : "", "II");
        break;
    case sf_mp1:
        g_skip_start += 240 + 1;
        if (silent < 10)
            console_printf("MPEG-%u%s Layer %s", 2 - lame_get_version(g_gf),
                           lame_get_out_samplerate(g_gf) < 16000 ? ".5" : "", "I");
        break;
    case sf_raw:
        if (silent < 10)
            console_printf("raw PCM data");
        mp3input_data.nsamp = lame_get_num_samples(g_gf);
        mp3input_data.framesize = 1152;
        g_skip_start = 0; /* other formats have no delay *//* is += 0 not better ??? */
        break;
    case sf_wave:
        if (silent < 10)
            console_printf("Microsoft WAVE");
   ...
   // wavsize = -(skip_start + skip_end);
    g_WriteFunction = swapbytes ? WriteBytesSwappedToBuffer : WriteBytesToBuffer;
    mp3input_data.totalframes = mp3input_data.nsamp / mp3input_data.framesize;

    //assert(tmp_num_channels >= 1 && tmp_num_channels <= 2);		choi140115 temp del

	return readbytes;
}

int lame_Mp3Decode( unsigned char *pcmdata, const unsigned char *mp3data, int len )
{
    short int Buffer[2][1152];
    int iread;
	int i;
	...
	int idxMp3 = 0;
    do {
        iread = get_audio16_FromBuffer(g_gf, Buffer, mp3data, &idxMp3, len); /* read in 'iread' samples */
        if (iread >= 0) {
            mp3input_data.framenum += iread / mp3input_data.framesize;
            //wavsize += iread;
            ...
            g_skip_start -= (i = g_skip_start < iread ? g_skip_start : iread); /* 'i' samples are to skip in this frame */

            if (g_skip_end > 1152 && mp3input_data.framenum + 2 > mp3input_data.totalframes) {
                iread -= (g_skip_end - 1152);
                g_skip_end = 1152;
            }
            else if (mp3input_data.framenum == mp3input_data.totalframes && iread != 0)
                iread -= g_skip_end;
            ...
            for (; i < iread; i++) {
                //if (disable_wav_header) {
                    idxPcm += g_WriteFunction(pcmdata, idxPcm, (char *) &Buffer[0][i], sizeof(short));
                    if (g_tmp_num_channels == 2)
                        idxPcm += g_WriteFunction(pcmdata, idxPcm, (char *) &Buffer[1][i], sizeof(short));
                //}
                //else {
                //    Write16BitsLowHigh(outf, Buffer[0][i]);
                //    if (g_tmp_num_channels == 2)
...
void lame_DecodeExit()
{
	/*// choi090330 temp start
	int ret = 0;
	int imp3 = 0;
	unsigned char mp3buffer[LAME_MAXMP3BUFFER];

	imp3 = lame_encode_flush(g_gf, mp3buffer, sizeof(mp3buffer)); // may return one more mp3 frame
	...
	encoder_progress_end(g_gf);
    
	if (silent <= 0) {
		print_trailing_info(g_gf);
	}    
	ret = 0;
_inner_ret:;
